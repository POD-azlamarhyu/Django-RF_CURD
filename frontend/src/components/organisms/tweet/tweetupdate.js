...
const TweetUpdate= (tweet_id) => {
    ...
    const submitTweet = async (event) => {
        ...
        await axios.put(`http://0.0.0.0:8000/tweet/${tweet_id.match.params.id}`,form_data,{
            headers:{
            ...
            <div className="m-2">
                <form className="" onSubmit={submitTweet} >
                    ...
                    <div className="flex justify-end">
                        <button className="bg-blue-400 hover:bg-blue-500 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="submit">編集を保存</button>
                    </div>
                </form>
            </div>
        </div>
